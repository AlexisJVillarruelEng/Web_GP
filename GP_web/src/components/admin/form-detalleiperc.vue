  ...
  <q-card class="q-pa-md form-container">
    ...
    <q-card-section>
      ...
      <q-select v-model="partidaSeleccionada" :options="partidas" option-value="idPartida" option-label="nombrePartida"
        label="Seleccionar Partida" outlined dense emit-value map-options @update:model-value="cargarProcesos" />
    </q-card-section>
              ...
              <div class="tareas-contenedor">
                <div class="tarea-item" v-for="tarea in props.row.tareas" :key="tarea.idTarea">
                  ...
                  <div class="detalle-iperc-container">
                    <q-table v-if="tarea.detalleIPERC.length > 0" :rows="tarea.detalleIPERC"
                      :columns="columnasDetalleIPERC" dense class="tabla-detalle-iperc" />
    ...
    <q-card-section v-if="tareaSeleccionada">
      <div class="text-subtitle1">Tarea Seleccionada: {{ tareaSeleccionada.nombreTarea }}</div>

      <q-input v-model="detalleIPERC.descPeligros" label="Descripción de Peligros" outlined dense />
      <q-select v-model="detalleIPERC.tipoPeligro" :options="['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII']"
        label="Tipo de Peligro" outlined dense />
      ...
      <q-input v-model="detalleIPERC.procedimientosExistentes" label="Procedimientos Existentes" type="number"
        outlined dense />
